...
typedef struct {
    float **weights; // weights[fcOutputSize][fcInputSize] //EDIT: use 1D array instead of 2D for efficiency
...
// Function prototypes
float *linear_avx2(float *fcInput, float **weights, float *biases, int fcInputSize, int fcOutputSize); //NEW
...
// Implement the linear layer function with AVX2 optimization -- 8 floats simultaneously
float *linear_avx2(float *fcInput, float **weights, float *biases, int fcInputSize, int fcOutputSize) {
    float *output = (float *)malloc(fcOutputSize * sizeof(float));
    
    for (int i = 0; i < fcOutputSize; i++) {
        __m256 sum_vec = _mm256_setzero_ps();  // Set float vector to 0 as accumulator. _m256 type is for 8 floats
        
        int j = 0;
        for (; j <= fcInputSize - 8; j += 8) {
            __m256 input_vec = _mm256_loadu_ps(&fcInput[j]); //load 8 floats from input
            __m256 weight_vec = _mm256_loadu_ps(&weights[i][j]); //load 8 weights from input
            __m256 prod = _mm256_mul_ps(input_vec, weight_vec); //multiply
            sum_vec = _mm256_add_ps(sum_vec, prod); //accumulate
        }
        
        //now that we have the sum in 8 elements for the whole layer, we need to add them together to get one value
        __m128 sum_high = _mm256_extractf128_ps(sum_vec, 1); //extract first 4 lements
        __m128 sum_low = _mm256_castps256_ps128(sum_vec); //extract last 4 elements
        __m128 sum128 = _mm_add_ps(sum_low, sum_high); //add together, now have 4 elements
        //_mm_movehl_ps combines latter half of both arguments, with first argument being higher half in result
        __m128 sum64 = _mm_add_ps(sum128, _mm_movehl_ps(sum128, sum128)); //add together, now have 2 elements (& 2 garbage elemtents)
        //_mm_shuffle_ps:
        //mask is 0x1 which is 00 00 00 01 -- these select whether take from 1st or 2nd argument for
        //  each of 4 output elements -- 00 00 selects from vector b for positions 3 and 2, 00 01 selects from vector a for position 1 and 0
        //  so we get element 1 (the second element) of sum64 in the lower position
        __m128 sum32 = _mm_add_ss(sum64, _mm_shuffle_ps(sum64, sum64, 0x1));  //then we add together as usual to get 1 float in first position
        float sum = _mm_cvtss_f32(sum32); //convert first float sized element in 128 bits to float
        
        //handle remaining elements as normal if not neatly divisible by 8
        for (; j < fcInputSize; j++) {
            sum += fcInput[j] * weights[i][j];
        }
        
        output[i] = biases[i] + sum; //add bias
    }
...
float **block(float **x, int seqLength, int embeddingSize, BlockWeights weights) {
    ...
    for (int i = 0; i < seqLength; i++) {
        Q[i] = linear_avx2(normalized_x[i], q_mlp.weights, q_mlp.biases, q_mlp.fcInputSize, q_mlp.fcOutputSize); //switched
        K[i] = linear_avx2(normalized_x[i], k_mlp.weights, k_mlp.biases, k_mlp.fcInputSize, k_mlp.fcOutputSize); //switched
        V[i] = linear_avx2(normalized_x[i], v_mlp.weights, v_mlp.biases, v_mlp.fcInputSize, v_mlp.fcOutputSize); //switched
    }
    ...
    for (int i = 0; i < seqLength; i++) {
        ...
        //switched to avx2
        float *hidden = linear_avx2(normalized_x_added[i], first_block_MLP.weights, first_block_MLP.biases, first_block_MLP.fcInputSize, first_block_MLP.fcOutputSize);
        float *activated = gelu(hidden, first_block_MLP.fcOutputSize);
        m[i] = linear_avx2(activated, second_block_MLP.weights, second_block_MLP.biases, second_block_MLP.fcInputSize, second_block_MLP.fcOutputSize);
        free(hidden);
        free(activated);
    }
...
float *model(int *tokens, int seqLength, GPT2Weights weights) {
    ...
    float *logits = linear_avx2(h[seqLength - 1], logits_mlp.weights, logits_mlp.biases, logits_mlp.fcInputSize, logits_mlp.fcOutputSize);
...
void initialize_linear_layer(LinearLayer *layer, int inputSize, int outputSize) {
    ...
    //EDIT: linear weight allocation to align data structures to cache line boundaries
    // posix_memalign((void**)&L->weights, 32, sizeof(float) * inputSize * outputSize);
    // posix_memalign((void**)&L->biases, 32, sizeof(float) * outputSize);
